...
> `./ec cleanup` = 잔재 폴더 정리, `./ec manual` = 수동 캡처,
> 그 외 인자는 collect.py로 그대로 전달 (예: `./ec --auto --menus 공지,과제`).

### 터미널 없이 실행 (더블클릭 앱)
프로젝트 루트의 `eCampus 수집.app` 더블클릭 = `./ec` 자동 수집. 끝나면 macOS 알림으로
"완료: 메뉴 N개 · 게시물 N개 …" 요약이 뜨고, 오류이거나 완료 요약이 없으면(로그인/과목
진입 실패 등) 로그 파일을 열어준다. 이미 수집이 돌고 있으면 중복 실행하지 않음.
- **선행 조건: `./ec login` 1회**(키체인 등록). 앱에는 stdin이 없어 수동 로그인 폴백이
  불가능 — 미등록이면 시작 전에 알림으로 알려주고 아무것도 하지 않는다.
- Dock에 끌어다 놓으면 한 번 클릭으로 실행 가능
- 실행 로그: `output/_logs/run-<시각>.log`
- 앱은 빌드 산출물(gitignore) — 소스는 [ec-app.applescript](ec-app.applescript), 수정하면 재빌드:
  `rm -rf "eCampus 수집.app" && osacompile -o "eCampus 수집.app" ec-app.applescript`
  (기존 .app 위에 덮어 빌드하면 코드서명 seal이 깨져 "열 수 없습니다"로 실행 거부됨 — rm 필수)

### 0) 자동 로그인 등록 (1회, 선택이지만 강력 추천)
```bash
./ec login     # 아이디/비밀번호 → macOS 키체인에 저장 (eCampus 로그인 화면에 치는 그 계정 — 건국대 포탈 계정과 연동)
```
...
### 잔재 폴더 정리 (`./ec cleanup`)
초기 버전은 게시물 폴더명에 목록의 작성자·조회수 꼬리표가 그대로 들어가
(`02_…공지 정갑주 조회 118`), 조회수가 바뀔 때마다 같은 글의 폴더가 새로 쌓였다.
지금은 제목을 정리해 저장하지만 옛 폴더는 남아 있으므로:
- **`./ec cleanup`**: 수집 없이 `output/` 전체의 잔재만 정리.
- `--auto` 도 메뉴 진입 때마다 같은 정리를 자동 수행(자가 치유).

정리 규칙: 각 폴더의 `meta.json`(원본 제목 + 글 식별자 URL)을 근거로 같은 글의
중복 폴더를 **최신본에 병합**(다운로드 파일은 모두 보존, 같은 이름·다른 크기는 `_2`로)
후 제거하고, 폴더명을 현행 규칙(`<글ID>_<제목 40자>`)으로 통일한다.
`meta.json` 없는 폴더와 `_archive*` 는 건드리지 않는다. 제목이 바뀐 글은
다음 수집에서 같은 글ID의 폴더를 자동 개명해 재사용한다.
...
> 이미 받은 첨부는 `attachments.json`·`submission.json`의 FILE_SEQ/token으로 알아보고
> 건너뜁니다(일반 게시물 첨부 + 팀프로젝트 제출 첨부 모두). 파일이 실제로 디스크에
> 있을 때만 — 지웠으면 다시 받음. 예전 형식(`01_제목 … 조회 N`) 폴더에 받아둔 파일도
> 그대로 인식해 재다운로드하지 않습니다.
>
